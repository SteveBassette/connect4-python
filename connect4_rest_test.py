...
from FlaskTestAdaptor import FlaskServerSession, FlaskClientAdaptor

class TestConnect4RestClient(Connect4CLITestCase):

    def setUp(self) -> None:
        connect4_rest.app.testing = True
        self.client = connect4_rest.app.test_client()
        self.session = FlaskServerSession(self.client, "http://127.0.0.1")
        self.session.mount("http://", FlaskClientAdaptor(self.client))
    ...
    def test_user_can_start_a_game(self):
        client = connect4_rest.Connect4Client(self.session)
